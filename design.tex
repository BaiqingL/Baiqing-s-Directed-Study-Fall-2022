...
\begin{abstract}
This document lays out the inspiration for a re-configurable data stream processor.
...
framework is that it is designed to be re-configurable, and supports better scalability
and fault tolerance for stateful applications compared to current industry standard solutions.
\end{abstract}
...
is to create a stream processor that has a certain tradeoff which can be expressed of having a bit 
higher latency than systems like Apache Flink, but lower than Apache Spark, including better scalability 
than those frameworks in addition to having capabilities for re-configuration.
...
\begin{enumerate}
\item Create MVP of a ring dataflow backend that allow nodes to come in and out of the 
data processor.

\item Give the ability to input data into the dataflow and have a sink that can recieve 
the output.

\item Develop a MVP of a data stream processor that can support easy map reduce operations 
(i.e. wordcount)
\end{enumerate}
...
Using the DHT implementation specified by Chord, we will modify it to support the map reduce 
functionality. Instead of the traditional setting of value to a node, there will be new methods 
that act as operators. For example instead of \texttt{setValue(value)} and letting Chord handle 
the corresponding node, we will have \texttt{sumOperator(value)} and let Chord set the value, 
and instruct the node to perform the sum operation. Similarly, we could have custom operators to 
perform any actions needed, as it will be chained as a secondary set to after storing the data in their
corresponding nodes.

\subsection{Ring Design}
The stream processor will maintain a ring of nodes. Each node contains a finger table containing
up to $m$ entries, $m$ being the number of bits in the hash key. Each $i^{th}$ entry in the finger table
of node $n$ contains the next node $successor$ which is the $((n + 2 ^ {i - 1})\ mod\ 2^m)$ node.

\subsection{Event routing}
Each time there is a new event from the data source, a node lookup event is distributed to a node in the
ring in a round-robin fashion. The node lookup event contains information regarding the key of the event.
Then, the node that recieves the event $e$ will respond to the source with their id so that the source can
contact and send the event to the correct node. This is done to prevent bouncing the entire data event within
the ring sturcture, which can induce high IO.


\subsection{Ring stablization}
To ensure that the ring is always up to date, some sort of stablization mechanism must be run periodically in
the background so that the finger tables of the nodes are always correct.

\subsubsection*{Ring joins}

\subsubsection*{Ring leaves}

\subsubsection*{Heartbeat Check}
